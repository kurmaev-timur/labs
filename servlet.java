...
import com.con1.config.Con1ContextPlugin;
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (cmd.equals("updateTemp")) {
			...
			try {
				...
				temp = con1.getVariable("temp_array_table").clone().rec().getString("temp_array");
		...
		if (cmd.equals("updateHum")) {
			...
			try {
				CallerController uncheckedCallerController = new UncheckedCallerController();
				Context con1 = Server.getContextManager().get("con1.test1", uncheckedCallerController);
				hum = con1.getVariable("hum_array_table").clone().rec().getString("hum_array");
			} catch (ContextException ex) {
				Con1ContextPlugin.LOGGER.warn(ex.getMessage());
			}
			reply = hum;
